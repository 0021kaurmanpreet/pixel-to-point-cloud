...
def _distort_pixels(
    ...
    # Radial distortion
    numerator = 1 + distortion_coefficients.k1 * r2 + distortion_coefficients.k2 * r4 + distortion_coefficients.k3 * r6
    denominator = 1 + distortion_coefficients.k4 * r2 + distortion_coefficients.k5 * r4 + distortion_coefficients.k6 * r6
    radial = numerator / denominator
    radial_distortion = normalized_pixels * radial[..., None]

    # Tangential distortion
    two_xy = 2 * x * y
    dx = distortion_coefficients.p1 * two_xy + distortion_coefficients.p2 * (r2 + 2 * x**2)
    dy = distortion_coefficients.p2 * two_xy + distortion_coefficients.p1 * (r2 + 2 * y**2)
    ...
    if tx != 0.0 or ty != 0.0:
        ...
        padded = np.concatenate([distorted, np.ones((*distorted.shape[:2], 1), dtype=np.float32)], axis=-1)
